...
while [[ $validToken != "true" ]]; do
  echo -e "Token Invalid, Please Double Check and Enter Token Again \n"
  read -p 'Paste API Token here: ' apiToken # Get API Token
  validToken=$(curl -s -X GET "https://api.cloudflare.com/client/v4/user/tokens/verify" \
                      -H "Authorization: Bearer $apiToken" \
                      -H "Content-Type:application/json" | jq .success)
...
if [[ $(echo $apiCall | jq .success) != true ]]; then # Error Checking
  echo "API call Failed. This may be caused by either Cloudflare's API server error or poor connectivity of your server"
  echo "Please try again later"
  exit
fi
zoneCount=$(echo $apiCall | jq .result_info.count)
echo "Found ${zoneCount} zones"
...
for zoneID in ${zoneIDs[@]}; do
  ...
  while [[ $index -lt $recordCount ]]; do
    ...
    while :; do
      ...
      case "$response" in
        [yY])
          updateTarget=$updateTarget","$(echo "{\"name\": \"$(echo $apiCall | jq -r .result[$index].name)\", \"id\": \"$(echo $apiCall | jq -r .result[$index].id)\", \"zone_id\": \"$(echo $apiCall | jq -r .result[$index].zone_id)\"}" | jq .)
          count=$[$count+1]
...
# Write JSON File
echo $(echo "{\"api\": \"$apiToken\", \"update-target\": ["${updateTarget:1}"]}" | jq .) > /opt/cloudflare-dns-update/config/config.json
echo "Config file saved as /opt/cloudflare-dns-update/config/config.json"
echo $count" DNS record will be updated"
